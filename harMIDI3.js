...
// Playing notes
	...
	//Playing the main melody
		...
		function computeSlowerDUration(){
	...
	//Playing MinNote and First Species Ctp notes
		...
		function buildMinNotesSecond(){

			min_note_second.length = ctp2_array.length;	
			var i = first_min_index;
			var j = first_min_index;
			while (i<min_note_second.length){
				min_note_second[i] = min_note_array[j];
				min_note_second[i+1] = min_note_array[j];
				chordOnMin[i] = chordOn[j];
				chordOnMin[i+1] = chordOn[j];
				i = i+2;
				j = j+1;
			}
			return min_note_second;
		}
		...
		function startPlayingSecondCtp(){
			...
			if (y<ctp2_array.length && second == true){
				...
				if(ctp2On[y]){
					...
					console.log("keyb_chord_freq = " + keyb_chord_freq + " harm_duration = " + ctp2_duration[y] + " ON"+ " y= " +y);
				} else{
