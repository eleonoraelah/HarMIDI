...
// If send is false, so if we stopped recording
	//creates two arrays, one containing the minimum note for each chord, one containing its timing information
		...
		function getMinNote(){
			...
			min_note = chord_array[first_chord_index][1];
			...
			min_note_array[min_note_i] = chord_array[min_note_index];
			min_note_time[min_note_i] = chord_time[min_note_index];
			last_chord_index = chord_array.length;
			min_note_i = min_note_array.length;
			m++;
		}
	...
	// Creating the duration arrays for the First and Second Species Counterpoints
		...
		function getChordDuration(){
			...
			// Second Species Ctp array	
				...
				//min notes
				min_note_second[ctp2_i] = min_note_array[chord_i];
				min_note_second[ctp2_i+1] = min_note_array[chord_i];
			}
		}
...
// Playing notes
	...
	//Playing the main melody
		...
		function startMelody(){
			if (k<melody_array.length){
				...
				if(noteOn[k]){
					...
					console.log(midi_freq + " = " + keyb_freq + " durata " + melody_duration[k]*cost + " ON" + " k = " + k);
				} else{
					console.log(midi_freq + " = " + keyb_freq + " durata " + melody_duration[k]*cost + " OFF" + " k = " + k);
				}
				setTimeout(startMelody, melody_duration[k]*cost);
				k++;
			}else {
				if (!harmony){
					play_button_clicked.classList.toggle("clicked_button");
		...
		function playMelody(freq){
		    ...
		    current_dur = (melody_duration[k]*cost)/4000;
		    ...
		    osc.stop((melody_duration[k]*cost)/1000);
	...
	//Playing MinNote and First Species Ctp notes
		...
		function startPlayingSecondCtp(){
			...
			if (y<ctp2_array.length){
				...
				if(ctp2On[y]){
					...
					console.log("midi_freq_second = " + keyb_freq_second + " harm_duration = " + ctp2_duration[y]*cost + " ON"+ " y= " +y);
					console.log("keyb_chord_freq = " + keyb_chord_freq + " harm_duration = " + ctp2_duration[y]*cost + " ON"+ " y= " +y);
				} else{
					console.log("midi_freq_second = " + keyb_freq_second + " harm_duration = " + ctp2_duration[y]*cost + " OFF"+ " y= " +y);
				}
		...
		function playSecondCtp(freq1,freq2){
			...
			osc_chord.stop((ctp2_duration[y]*cost)/1000);
		}
